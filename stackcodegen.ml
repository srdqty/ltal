...
let current_rettp t2 benv = 
  ...
  (* rt is caller-save *)
  let rctx = update rctx rt toptp in
...
let declare_label l ltp benv = 
  ...
  {cenv={cctx=cctx';cs=benv.cenv.cs;lctx=benv.cenv.lctx;fctx=benv.cenv.fctx;fp = benv.cenv.fp};
   ilist = benv.ilist;
   lab = benv.lab;
   tctx= benv.tctx;
   rctx=benv.rctx}
...
let emit_end ei benv  = 
  try 
    ...
    {cctx = cenv.cctx; cs = cs'; fctx = cenv.fctx; lctx = cenv.lctx; fp = cenv.fp}
  with TcFail msg -> raise (BlockFail (msg,benv))
...
let push r s i = (emit (PUSH(r,s,i)))
let pop r s i  = (emit (POP(r,s,i)))
...
let begin_program lstart tp = 
  let ltp = (Code(Ctx.from_list[(rs,Stack(One));
			       (ra,toptp);
			       (rt,toptp);
			       (rf,listtp);
...
let inc_fp i benv = 
   {cenv={cctx=benv.cenv.cctx;cs=benv.cenv.cs;lctx=benv.cenv.lctx;fctx=benv.cenv.fctx;fp = benv.cenv.fp + i};
   ilist = benv.ilist;
   lab = benv.lab;
   tctx= benv.tctx;
   rctx=benv.rctx}

let dec_fp i benv = inc_fp (-i) benv

let get_fp cenv = cenv.fp
...
(* TODO: Factor out case and if0 into the case used by malloc. *)
  ...
  in 
  let cenv = codegen_outer e tp {cctx=emp;cs=emp;fctx=emp;lctx=emp;fp=0} in (**)
